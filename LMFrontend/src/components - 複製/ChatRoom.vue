<template>
  ...
  >
    ...
    <!-- RAG 檔案拖曳天藍色科技感遮罩 -->
    <div v-if="isDragging && !chatStore.currentAgentId" class="absolute inset-0 z-50 bg-[#e0f2fe]/95 backdrop-blur-md flex flex-col items-center justify-center border-4 border-dashed border-[#3b82f6] rounded-2xl transition-all duration-200 shadow-2xl">
    ...
    <div class="flex-1 flex flex-col h-full bg-[#f4f8fc] overflow-hidden">
      ...
      <div class="chat-header py-4 px-6 bg-white/90 backdrop-blur-md border-b border-[#e2e8f0] flex items-center justify-between shrink-0 shadow-2xs relative z-10">
        <div class="flex items-center gap-3">
          <div class="w-3 h-3 rounded-full" :class="chatStore.currentAgentId ? 'bg-[#67c23a] shadow-[0_0_8px_#67c23a]' : 'bg-[#3b82f6] animate-pulse shadow-[0_0_8px_#3b82f6]'"></div>
          <h1 class="text-base font-bold text-[#0f172a] tracking-wide">{{ currentRoomTitle }}</h1>
        </div>
        
        <span v-if="chatStore.currentAgentId" class="text-xs text-[#16a34a] bg-[#f0fdf4] border border-[#bbf7d0] px-3 py-1 rounded-full font-semibold flex items-center gap-1.5 shadow-2xs">
      ...
      <!-- 對話內容展示滾動區 (調整寬度黃金分割比，左右留白更有呼吸感) -->
      <el-scrollbar class="chat-window flex-1" ref="scrollbarRef">
        <div v-if="!chatStore.currentRoomId" class="h-full flex flex-col items-center justify-center text-[#94a3b8] gap-3 py-32">
          <span class="text-5xl animate-pulse">👋</span>
          <p class="text-sm font-medium">歡迎使用！請在左側選擇或建立一個聊天室開始深度對話。</p>
        </div>

        <!-- 🚀 視覺微調：改為 max-w-4xl，讓泡泡長度符合人眼閱讀習慣，不再空曠散亂 -->
        <div v-else class="chat-content-inner p-6 max-w-4xl mx-auto flex flex-col gap-6 w-full">
          
          <!-- ⚡ 終端機日誌面板：融入發光極客黑客感，改善原圖空隙過大問題 -->
          <div v-if="chatStore.isLoading" class="cli-logs-container bg-[#0b0f17] rounded-xl p-4 border border-[#67c23a]/40 shadow-[0_0_15px_rgba(103,194,58,0.1)] font-mono text-xs text-[#67c23a] flex flex-col gap-2 transition-all">
            ...
            <div class="text-[#a6e22e] flex items-center gap-2">⚡ <span class="text-[#e2e8f0]">Executing:</span> hermes chat --agent {{ chatStore.currentAgentId || 'default' }}</div>
          ...
          <div v-if="chatStore.isLoading" class="flex w-full gap-4 flex-row">
            ...
            <div class="bubble px-4 py-3 rounded-2xl rounded-tl-none border border-[#e2e8f0] bg-white text-[14px] text-[#475569] flex items-center gap-2.5 shadow-2xs">
              <el-icon class="is-loading text-[#3b82f6]" :class="{ 'text-[#67c23a]': chatStore.currentAgentId }"><Loading /></el-icon> 
    ...
    ></div>
    <!-- ========================================================================= -->
    <!-- 【右欄】智能體核心主控台 (解鎖固定寬度，完美升級微藍科技玻璃質感) -->
    <!-- ========================================================================= -->
    <div 
  ...
  <!-- ========================================================================= -->
  <!-- ➕ 【配置 Hermes 底層自訂模型】技術規格表單彈窗組件 (改版為弧形大氣科技感) -->
  <!-- ========================================================================= -->
  <el-dialog 
...
// =========================================================================
const sidebarWidth = ref(320); // 預設加寬至 320px，更顯大氣大方
...
const handleFileDrop = async (e) => {
  ...
  const targetFile = files[0]; // 修正精準指向第一個上傳檔案
...
/* ========================================================================= */
/* 🎛️ Element Plus 深度科技化樣式優化 (消除慘白與過時粗糙感) */
/* ========================================================================= */
