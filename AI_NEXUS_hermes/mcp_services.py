...
def build_hermes_mcp_servers_block(agent_id: str) -> dict:
    ...
    for name, entry in servers.items():
        ...
        # 🐛 修正：之前不管有沒有真的設定過值，母版宣告的 credentialFields 一律塞進
        # env_refs，導致「選填但沒人填過」的欄位（例如 PHISON_API_URL）也會被寫進
        # config.yaml 變成 ${MCP_..._PHISON_API_URL}——hermes 找不到這個環境變數時
        # 會把它解析成空字串（不是不設定、也不是保留原樣），空字串會覆蓋掉 MCP
        # 腳本自己程式碼裡寫的合理預設值，導致腳本壞掉（實測過：phison_mcp_bridge.py
        # 的 PHISON_API_URL 就是這樣被空字串蓋掉，出現「缺少 URL scheme」的錯誤）。
        # 修法：只有「真的被設定過值」的欄位才產生 ${...} 參照；沒設定過的欄位乾脆
        # 不放進 env，讓 MCP 腳本自己 os.getenv(KEY, 合理預設值) 的 fallback 生效。
        credential_fields = entry.get("credentialFields", [])
        credentials_configured = entry.get("credentialsConfigured", {})
        env_refs = {
            f["key"]: f"${{{_env_var_name(name, f['key'])}}}"
            for f in credential_fields
            if credentials_configured.get(f["key"])
        }
